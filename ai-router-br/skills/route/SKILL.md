...
2. **Decida — o veredito do router manda.**
   ...
   - `status: blocked` (`dirty_worktree`, `tracked_secret`, `tracked_router_config`, `not_git_repo`, `unsafe_test_command`, ...) ou `failed` sem fallback: não contorne o gate; siga no principal ou peça ao usuário o que só ele pode resolver. Diga ao usuário o motivo concreto que vem em `error` (ex.: `tracked_secret:.env.production`), não só o código.
