...
import { ensureInitialized, resolveProjectRoot } from '../lib/bootstrap.mjs';
...
try {
  if(cmd==='init') {
    ...
    if(cmd==='new-task'){ const p=saveTask(root,{objective:flag('objective',''),allowed_files:list(flag('allowed')),forbidden_files:list(flag('forbidden'))}); print({status:'task_created',task:p,auto_init}); }
      ...
      print({...res,auto_init,...(ignored.length?{config_ignored_settings:ignored}:{})});
    }
  } else {
  ...
  print({status:'error',error:String(e.message||e),error_code:e.code||null});
  process.exit(1);
}
